...
Another example, an image of a bonsai, has final output shown below:

<img src="assets/img/Bonsai.jpeg" />

#### Bonus visual output: Generating Voronoi diagrams from the points in `points.csv`
The points generated in `points.csv` also can be taken as the centroids of Voronoi cells to create a nice Voronoi diagram (which can then also be laser-etched):

<img src="assets/img/Bird1.jpeg" />

The Voronoi cells are generated by importing the `csv` into Grasshopper, and then simply using the Voronoi component in Rhino with a bounding box to limit the cells to a frame.

### Reflection
I am somewhat proud of the program that generates the output, but I think the visual output is not particularly striking without understanding the code.
The program is also less robust than I would hope, because it seems to only produce interpretable images when the image is *extremely* high contrast; i.e., with a white background and a single subject.
For example, see the below output for an image of trees in snow (which already seems like a visually high contrast image):

<img src="assets/img/trees.jpg" />
<img src="assets/img/treepoints.png" />

which generates nearly-incomprehensive line output. 

Robert Bosch's works are far more impactful visually; he also writes that he uses the [Concorde TSP Solver](https://www.math.uwaterloo.ca/tsp/concorde.html), which is the fastest exact TSP solver, and still takes several hours to run on his machine, which is surely more powerful than mine.
This suggests that to achieve truly impressive results, I would probably need far more time and compute power.
However, as a proof of concept, it was interesting to bring this algorithm to life.
